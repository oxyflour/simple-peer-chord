...
Chord.prototype.call = function *(id, method, arg) {
	return id === this.id ?
		yield *this.node.call(method, arg) :
		yield *this.hub.call(id, method, arg)
}
...
Chord.prototype.connectViaProxy = function *(id, proxyId, token, initiator) {
	...
	var handler
	this.on('-chord-signal-offer', handler = data => {
		data.token === token && peer.signal(data.offer)
		this.hub.has(id) && this.removeListener('-chord-signal-offer', handler)
	})
...
Chord.prototype.run = function() {
	if (!this.running) return co(function *() {
		...
		var checkCalls = setInterval(_ => {
			this.hub.checkCalls()
		}, this.opts.stabilizeInterval)

		while (this.running) {
			try {
				yield this.node.poll().concat(sleep(this.opts.stabilizeInterval))
