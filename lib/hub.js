...
class Hub {
	...
	recycle () {
		var now = Date.now(), ids = Object.keys(this.conns)
	...
	* get (id) {
		...
		if (!this.conns[id]) {
			...
			var time = Date.now(), err = new Error('timeout')
	...
	* query (id, method, arg) {
		var peer = yield* this.get(id), time = Date.now(), err = new Error(this.id + ' query with ' + id + ' (' + method + ') timeout'), req = id + '#' + Math.random() + '#' + time
