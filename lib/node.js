...
Node.prototype.join = function *(bootstrapId) {
	...
	var fingerIds = [ successorId ]
	for (var i = 1; i < this.fingerIds.length; i ++) {
		var id = NodeId.fingerStart(this.id, i)
		fingerIds[i] = yield *this.conn.query(bootstrapId, 'findSuccessorId', id)
		yield *this.conn.notify(fingerIds[i], 'successorId')
	}

	// connect to the successor to sync the storage
	var storedKeys = yield *this.conn.query(successorId, 'keys'), storage = { }
	for (var i = 0; i < storedKeys.length; i ++) {
		var key = storedKeys[i]
		if (this.isResponsibleFor(key)) {
			storage[key] = yield *this.conn.query(successorId, 'get', key)
		}
	}

	// connect to more successors in case of failures
	for (var i = 0; i < succBackupIds.length && i < this.opts.succBackupCocurrency; i ++) {
		yield *this.conn.notify(succBackupIds[i], 'id')
	}

	// ok to join
	Object.assign(this, { succBackupIds, predecessorId, predBackupIds, fingerIds })
	Object.keys(storage).forEach(key => this._put(key, storage[key]))
}
