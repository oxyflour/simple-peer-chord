...
const sha1 = val => crypto.createHash('sha1').update(JSON.stringify(val)).digest().toString('hex')
...
Node.prototype.put = function *(id, val) {
	id = NodeId.create(sha1(id))
	return yield *this.lookup(id, 'put', { id, val }, this.opts.lookupTTL)
}

Node.prototype.get = function *(id) {
	id = NodeId.create(sha1(id))
	return yield *this.lookup(id, 'get', id, this.opts.lookupTTL)
}

Node.prototype.contains(id) {
	return NodeId.inRange(this.id, id, this.successorId) || id === this.successorId
}
...
Node.prototype._get = function(id) {
	var data = this.storage[id]
	return data && (data.active = Date.now()) && data.val
}

Node.prototype._hash = function(id) {
	var data = this.storage[id]
	return data && (data.active = Date.now()) && data.hash
}

Node.prototype._put = function(id, val) {
	var active = Date.now(), hash = sha1(val)
	this.storage[id] = { val, active, hash }
}
...
Node.prototype._recycleStore = function() {
	...
	this._keys().forEach(id => {
		if (!(this.storage[id].active > timeout))
			delete this.storage[id]
	})
}

Node.prototype.replicate = function *(successorId, id) {
	var data = this._get(id),
		val = data && data.val,
		hash = yield *this.pull(successorId, 'hash', id)
	if (id && data && hash !== data.hash)
		yield *this.pull(successorId, 'put', { id, val })
}
...
Node.prototype.join = function *(bootstrapId) {
	...
	// connect to the predecessor to sync the storage
	var storeKeys = this.predecessorId ? yield *this.pull(this.predecessorId, 'keys') : [ ]
	for (var i = 0; i < storeKeys.length; i ++) {
		if (this.contains(storeKeys[i]))
			this._put(id, yield *this.pull(this.predecessorId, 'get', id))
	}
...
Node.prototype.pull = function *(id, method, arg) {
	if (id === this.id) {
		if (method === 'id')
			...
			return this._put(arg.id, arg.val)
		else
...
Node.prototype.poll = function() {
	var fingerSize = this.fingerIds.length - 1,
		...
		successorsToSync = this._succWithBackupIds().slice(0, this.opts.succBackupCocurrency + 1),
		localStorageKeys = this._keys().filter(id => this.contains(id)),
