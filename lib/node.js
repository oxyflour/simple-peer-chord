...
Node.prototype.poll = function() {
	var fingerSize = this.fingerIds.length - 1,
		...
		succToSyncStorage = this._succWithBackupIds().slice(0, 1 + this.opts.succBackupCocurrency),
		localStorageKeys = this._keys().filter(id => NodeId.inRange(this.id, id, this.successorId) || id === this.successorId),
		idToSync = localStorageKeys[Math.floor(Math.random() * localStorageKeys.length)],
		syncQueries = succToSyncStorage.map(id => this.replicate(id, idToSync)),
