...
Node.prototype.join = function *(bootstrapId) {
	...
	var fingerIds = yield this.fingerIds.map((id, i) => NodeId.fingerStart(this.id, i))
			.map(id => this.conn.query(bootstrapId, 'findSuccessorId', id))
	yield fingerIds.map(id => this.conn.notify(id, 'id'))

	// connect to the successor to sync the storage
	var storedKeys = (yield *this.conn.query(successorId, 'keys')).map(key => this.isResponsibleFor(key)),
		storedVals = storedKeys.map(key => this.conn.query(successorId, 'get', key))
