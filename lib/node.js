...
Node.prototype.join = function *(bootstrapId) {
	this.successorId = yield *this.conn.query(bootstrapId, 'findSuccessorId', this.id)
	this.succBackupIds = yield *this.conn.query(this.successorId, 'succWithBackupIds')

	this.predecessorId = yield *this.conn.query(bootstrapId, 'findPredecessorId', this.id)
	this.predBackupIds = yield *this.conn.query(this.predecessorId, 'predWithBackupIds')

	// connect to the successor to sync the storage
	var storedKeys = yield *this.conn.query(this.successorId, 'keys')
	for (var i = 0; i < storedKeys.length; i ++) {
		var key = storedKeys[i]
		if (this.isResponsibleFor(key)) {
			this._put(key, yield *this.conn.query(this.successorId, 'get', key))
		}
	}
	...
	for (var i = 1; i < this.fingerIds.length; i ++) {
		var id = NodeId.fingerStart(this.id, i)
		this.fingerIds[i] = yield *this.conn.query(bootstrapId, 'findSuccessorId', id)
		yield *this.conn.notify(this.fingerIds[i], 'successorId')
	}

	// connect to more successors in case of failures
	for (var i = 0; i < this.succBackupIds.length && i < this.opts.succBackupCocurrency; i ++) {
		yield *this.conn.notify(this.succBackupIds[i], 'id')
	}
}
