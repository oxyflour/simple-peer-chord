...
function Node(opts, conn) {
	...
	this.predBackupIds = [ ] // does not include this.predecessorId
...
Node.prototype.stablize = function *() {
	...
	this.succBackupIds = (yield *this.conn.query(this.successorId,
		'checkSuccBackupIds', NodeId.hash(this.succBackupIds))) || this.succBackupIds
	this.predBackupIds = (yield *this.conn.query(this.successorId,
		'checkPredBackupIds', NodeId.hash(this.predBackupIds))) || this.predBackupIds
...
Node.prototype._subscribe = function(key, id) {
	var subs = this._getRecycledSubscriptions(key)
	subs[id] = Date.now()
	this._put(key, subs)
	return subs
}
...
Node.prototype._getRecycledSubscriptions = function(key) {
	var dict = this._get(key) || { },
		expiration = Date.now() - this.opts.subscriptionExpiration,
		subs = { }
	Object.keys(dict)
		.filter(id => dict[id] > expiration)
		.forEach(id => subs[id] = dict[id])
	return subs
}
...
Node.prototype.join = function *(bootstrapId) {
	var successorId = yield *this.conn.query(bootstrapId, 'findSuccessorId', this.id),
		succBackupIds = yield *this.conn.query(successorId, 'checkSuccBackupIds'),
		predecessorId = yield *this.conn.query(bootstrapId, 'findPredecessorId', this.id),
		predBackupIds = yield *this.conn.query(predecessorId, 'checkPredBackupIds')
...
Node.prototype.query = function *(method, arg) {
	...
	else if (method === 'checkSuccBackupIds') {
		return NodeId.hash(this._lastSuccList = this._successorList()) !== arg && this._lastSuccList
	}
	else if (method === 'checkPredBackupIds') {
		return NodeId.hash(this._lastPredList = this._predecessorList()) !== arg && this._lastPredList
	}
...
Node.prototype.poll = function *() {
	var fingerSize = this.fingerIds.length - 1,
		...
		keyToSync = localStorageKeys[Math.floor(Math.random() * localStorageKeys.length)],
		syncQueries = successorsToSync.map(id => this.replicate(id, keyToSync)),

		stablizationQuery = this.stablize()

	this._recycleStore()

	yield fingerQueries.concat(syncQueries).concat(stablizationQuery)
}
