...
Node.prototype.replicate = function *(successorId, id) {
	var hash = yield *this.pull(successorId, 'hash', id)
	if (hash !== this._hash(id)) {
...
Node.prototype.join = function *(bootstrapId) {
	...
	yield *this.pull(this.predecessorId, 'id')

	// connect to the predecessor to sync the storage
	var storedKeys = yield *this.pull(this.successorId, 'keys')
	for (var i = 0; i < storedKeys.length; i ++) {
		var id = storedKeys[i]
		if (this.isResponsibleFor(id))
			this._put(id, yield *this.pull(this.successorId, 'get', id))
	}
...
Node.prototype.lookup = function *(id, method, arg, ttl) {
	if (this.isResponsibleFor(id)) {
		...
		return nextId === this.id ?
			yield *this.pull(this.successorId, method, arg) :
