...
Node.prototype.put = function *(key, val, action) {
	key = NodeId.hash(key)
	return yield *this.lookup(key, 'put', { key, val, action }, this.opts.lookupTTL)
}
...
Node.prototype._put = function(key, val, action) {
	var data = this.storage[key]
	val = this.conn.onput(key, val, action, data && data.val)

	var active = Date.now(), hash = NodeId.hash(val)
...
Node.prototype.pull = function *(id, method, arg) {
	if (id === this.id) {
		if (method === 'id')
			...
			return this._put(arg.key, arg.val, arg.action)
		else
