...
Node.prototype.join = function *(bootstrapId) {
	...
	// connect the predecessor now
	this.predecessorId = yield *this.pull(bootstrapId, 'findPredecessorId', this.id)
	if (this.predecessorId) yield *this.pull(this.predecessorId, 'id')
...
Node.prototype.poll = function() {
	var fingerSize = this.fingerIds.length - 1,
		...
		succQueries = successorIndices.map(id => this.pull(id, 'successorId')),

		stablizationQuery = this.stablize()

	return fingerQueries.concat(succQueries).concat(stablizationQuery)
}
